...
if (isset($_GET["id"]) && !empty(trim($_GET["id"]))) {
    ...
    if ($stmt = $mysqli->prepare($sql)) {
        ...
        if ($stmt->execute()) {
            ...
            echo "There was an error obtaining game data.";
            header("location: index.php");
            exit();
        }
...
if ($stmtPromedio = $mysqli->prepare($sqlPromedio)) {
    ...
    if ($stmtPromedio->execute()) {
        ...
        echo "There was an error obtaining the average rating.";
    }
    $stmtPromedio->close();
} else {
    echo "Average rating query preparation error.";
}
...
if (!empty($promedio)) {
    ...
    if ($stmtUpdatePromedio = $mysqli->prepare($sqlUpdatePromedio)) {
        ...
        if ($stmtUpdatePromedio->execute()) {
            ...
            echo "There was an error updating the average rating.";
        }
        $stmtUpdatePromedio->close();
    } else {
        echo "Average rating query update error.";
    }
}
...
<header id="header" class="d-flex align-items-center" style="padding-bottom: 10px;">
      <div class="container d-flex align-items-center justify-content-between">  
        ...
        <nav id="navbar" class="navbar" style="width: fit-content">
          <ul>
            ...
            <li><a  class="nav-link scrollto" href="index.php?order=az">From A to Z</a></li>
    ...
    <div class="container mt-4">
        <div class="mb-12"><h1 class="text-center mb-4" style="font-size: 40px; font-weight: bold; margin-top: 90px">Details of: <span style="color: #8810ea;"><?php echo $titulo; ?></span></h1></div>
        <div class="row">
                       ...
                       if ($stmt->execute()) {
                           ...
                           if ($idUsuario !== 0) {
                               if (isset($usuariosEnBiblioteca[$idUsuario])) {
                                   ...
                                   echo "<p style='text-align: center; margin-top: 10px; color: #4405a0' font-weight: bold>This game is already in your library</p>";
                                   echo '<form action="bibliodelete.php" method="post">';
                                    ...
                                    echo '<button style="width: 100%; background-color: #4405a0; border-color: #4405a0; border-radius: 0 0 5px 5px" type="submit" class="btn btn-danger" title="Delete game" data-toggle="tooltip"><i class="bi bi-trash-fill"></i> Delete from library</button>';
                                    echo '</form>';
                               } else {
                                   ...
                                   echo '<button style="width: 100%; background-color: #8810ae; border-color: #8810ae; border-radius: 0 0 5px 5px" class="btn btn-primary" type="submit" id="save-biblio-btn"><i class="bi bi-bookmarks-fill"></i> Save in library</button>';
                                   echo "</form>";
                               }
                           } else {
                               echo "<p style='text-align: center;'>Log in to save this game in your library.</p>";
                           }
                       } else {
                           echo "<script>alert('There was an error, try again later.');</script>";
                       }
            ...
            <div class="col-md-8">
                ...
                <?php
                    if ($adm) {
                        echo "<a style='background-color: #8810ae; border-color: #8810ae; border-radius: 5px 5px 0 0; font-weight: bold' class='btn btn-success' href='editgame.php?id=" . $idJuego . "'>Edit game</a>";
                        echo "<form action='deletegame.php' method='post'>";
                        echo "<input type='hidden' name='idJuego' value='" . $idJuego . "'>";
                        echo "<button type='submit' class='btn btn-danger' style='width: 100%; background-color: #4405a0; border-color: #4405a0; border-radius: 0; font-weight: bold'>Delete game</button>";
                        echo "</form>";
                    }
                    
                ?>
                    <div class="card-body">
                        <p class="card-text"><strong style="color: #4405a0">Genre:</strong> <?php echo $genero; ?></p>
                        <p class="card-text"><strong style="color: #4405a0">Platforms:</strong> <?php echo $plataformas; ?></p>
                        <p class="card-text"><strong style="color: #4405a0">Developer:</strong> <?php echo $desarrollador; ?></p>
                        <p class="card-text"><strong style="color: #4405a0">Launch date:</strong> <?php echo $fechaLanzamiento; ?></p>
                        <p class="card-text"><strong style="color: #4405a0">Description:</strong> <?php echo $descripcion; ?></p>

                    </div>
                </div>
                <h2><strong>Average score:</strong> <?php echo round($promedio, 2); ?><i style='color: yellow; margin-left: 5px;' class='bi bi-star-fill'></i></h2>
                <hr>
        ...
        if ($stmtResenas = $mysqli->prepare($sqlResenas)) {
            ...
            if ($stmtResenas->execute()) {
                ...
                if ($totalResenas > 0) {
                    echo "<h2 style='font-weight: 400'>Reviews</h2>";
                    echo "<ul>";
                ...
                echo "There was an error fetching review data.";
            }
        ...
        if ($stmtUsuarioResena = $mysqli->prepare($sqlUsuarioResena)) {
            ...
            if ($stmtUsuarioResena->execute()) {
                ...
                if ($resultUsuarioResena->num_rows > 0) {
                    ...
                    echo "<input type='number' name='calificacion' value='".$rowResena['calificacion']."' min='1' max='5' placeholder='Rating (1-5)' style='width: 140px; border-width: 0; box-shadow: 0 0 5px #fff; padding: 3px; border-radius: 5px; background-color: #fff;' required>";
                    echo "<br>";
                    echo "<textarea style='margin-top: 10px; width: 70%; border-width: 0; box-shadow: 0 0 5px #fff; border-radius: 5px; background-color: #fff; padding: 3px' name='comentario' placeholder='Write your review... (optional)'>". $rowResena['comentario'] ."</textarea>";
                    echo "<br>";
                    echo "<input style='height: 30px; border-width: 0; box-shadow: 0 0 5px #a2e; border-radius: 5px; background-color: #a3f; color: white; font-weight: bold; padding: 3px' type='submit' value='Post review'>";
                    echo "</form>";
                    echo "<button style='height: 30px; margin-right: 20px; margin-bottom: 30px; border-width: 0; box-shadow: 0 0 5px #a2e; border-radius: 5px; background-color: #a3f; color: white; font-weight: bold; padding: 3px' id='edit-review-btn'>Edit review</button>";
                    ...
                    echo "<button style='height: 30px; border-width: 0; box-shadow: 0 0 5px #4405a0; border-radius: 5px; background-color: #4405a0; color: white; font-weight: bold; padding: 3px' id='delete-review-btn'>Delete review</button>";
                } else {
                    ...
                    echo "<input type='number' name='calificacion' min='1' max='5' placeholder='Rating (1-5)' style='width: 140px; border-width: 0; box-shadow: 0 0 5px #fff; padding: 3px; border-radius: 5px; background-color: #fff;' required>";
                    echo "<br>";
                    echo "<textarea style='margin-top: 10px; width: 70%; border-width: 0; box-shadow: 0 0 5px #fff; border-radius: 5px; background-color: #fff; padding: 3px' name='comentario' placeholder='Write your review... (optional)'></textarea>";
                    echo "<br>";
                    echo "<input style='height: 30px; border-width: 0; box-shadow: 0 0 5px #a2e; border-radius: 5px; background-color: #a3f; color: white; font-weight: bold; padding: 3px' type='submit' value='Post review'>";
                    echo "</form>";
                }
            } else {
                echo "There was an error verifying the review.";
            }

            // Cerrar la sentencia preparada
            $stmtUsuarioResena->close();
        } else {
            echo "Query prearation error.";
        }
    } else {
        echo "<p>Please log in to write reviews!</p>";
    }
    ?>
</div>
...
<script>
  ...
  <script>
    ...
    function cambiarIdioma() {
        window.location.href = "../game.php?id=" + idJuego;
    }
  </script>
</body>
</html>
