...
<body class="bg-dark">
  ...
  <div class="container mt-2 text-white text-center">
        <?php if (isset($_GET['order']) && $_GET['order'] == "az") {
          echo "<h1 class='tituloPrincipal'>GameVault's games from A to Z</h1>";
        } elseif (isset($_GET['order']) && $_GET['order'] == "rated") {
          echo "<h1 class='tituloPrincipal'>GameVault's best rated games</h1>";
        } elseif (isset($_GET['order']) && $_GET['order'] == "recent") {
          echo "<h1 class='tituloPrincipal'>GameVault's most recent games</h1>";
        } elseif (isset($_GET['busqueda'])) {
          echo "<h1 class='tituloPrincipal'>Searching on GameVault</h1>";
        } else {
          echo "<h1 class='tituloPrincipal'>GameVault's games</h1>";
        }
        ?>
        <form method="get" action="" class="mb-4" class="search-bar">
          <input type="text" name="busqueda" placeholder="Search game..." class="search-input">
          <button type="submit" class="search-button">Search</button>
          <a class="reset-button" href="index.php" style="color: white;">Reset</a>
        ...
        <div class="row">
            ...
            if (isset($_GET['busqueda']) && empty($terminoBusqueda)) {
              echo "<p class='mensajeError'>Waiting for a search...</p>";
            }elseif ($resultado->num_rows === 0) {
              // Aquí puedes mostrar un mensaje indicando que no se encontraron resultados
              echo "<p class='mensajeError'>This game isn't on GameVault</p>";
              echo "<p class='mensajeError'>If you would like to add it, click <a href='create.php'>here</a>.</p>";
            } else {
              ...
              while ($row = $resultado->fetch_assoc()) {
                ...
                <div class="col-md-3 mb-4">
                    <div class="card">
                        ...
                        <img src="<?php echo $fotoJuego ?>" alt="<?php echo $row["titulo"]?>" class="card-img-top">
                            <div class="card-body">
