    ...
    <section class="seccion-perfil-usuario">
        ...
        <div class="perfil-usuario-body">
            <div class="perfil-usuario-bio">
                ...
                <p class ="subtitulo">Tengo <?php echo $totalJuegos?> juegos añadidos!</p>
            </div>
            <table class="perfil-usuario-footer">
            <div class="filtro-botones">
              <button class="btn btn-danger" onclick="mostrarTodos()">Mostrar Todos</button>
              <button class="btn btn-primary" onclick="filtrarJuegos('Por jugar')">Por Jugar</button>
              <button class="btn btn-info" onclick="filtrarJuegos('En progreso')">En Progreso</button>
              <button class="btn btn-warning" onclick="filtrarJuegos('Completado')">Completado</button>
              <button class="btn btn-light" onclick="filtrarJuegos('Deseado')">Deseado</button>
          </div>
                <tr>
                    <th>Imagen</th>
                    <th>Titulo</th>
                ...
                if ($stmt = $mysqli->prepare($sql)) {
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            echo '<tr>';
                            echo '<td><img src="' . htmlspecialchars($row['fotoJuego']) . '" alt="'. htmlspecialchars($row['titulo']) .'" class="card-img-top"></td>';
                            echo '<td>' . htmlspecialchars($row['titulo']) . '</td>';
                            if ($row['estadoJuego'] == 1) {
                              echo '<td>Por jugar</td>';
                            ...
                            echo '<a href="game.php?id=' . $row['idJuego'] . '" class="btn btn-success" style="display: block; margin: 0 auto;" title="Ver Juego" data-toggle="tooltip"><i class="bi bi-eye-fill"></i></a>';
                            ...
                            if ($idUsuario == $idPerfil) {
                                ...
                                echo '<option value="1" ' . ($row['estadoJuego'] == 1 ? 'selected' : '') . '>Por jugar</option>';
                                echo '<option value="2" ' . ($row['estadoJuego'] == 2 ? 'selected' : '') . '>En progreso</option>';
                                echo '<option value="3" ' . ($row['estadoJuego'] == 3 ? 'selected' : '') . '>Completado</option>';
                                echo '<option value="4" ' . ($row['estadoJuego'] == 4 ? 'selected' : '') . '>Deseado</option>';
                                echo '</select>';
                                echo '<button type="submit" class="btn btn-info" style="display: block; margin: 0 auto;" title="Editar progreso" data-toggle="tooltip"  ><i class="bi bi-check-circle-fill"></i></button>';
                                echo '</form>';
                                echo '</div>';

                                echo '<button id="editButton_' . $row['idBibliotecaPersonal'] . '" class="btn btn-warning" style="display: block; margin: 0 auto;" title="Editar progreso" data-toggle="tooltip" onclick="toggleEditForm(' . $row['idBibliotecaPersonal'] . ');"><i class="bi bi-pencil-fill"></i></button>';

                                echo '<form action="bibliodelete.php" method="post">';
                                echo '<input type="hidden" name="idBibliotecaPersonal" value="' . $row['idBibliotecaPersonal'] . '">';
                                echo '<button type="submit" class="btn btn-danger" style="display: block; margin: 0 auto;" title="Eliminar Juego" data-toggle="tooltip"><i class="bi bi-trash-fill"></i></button>';
                                echo '</form>';
                            }
                            echo '</td>';
                            echo '</tr>';
                        ...
                        echo '<tr><td colspan="5">Biblioteca personal vacía, ve a conseguir juegos!</td></tr>';
                    }

                    $result->close();
                    $stmt->close();
                } else {
                    echo '<tr><td colspan="5">Error al llamar al procedimiento almacenado.</td></tr>';
                }
                ?>
            </table>
...
<script>
  ...
  <script>
    ...
    function cambiarIdioma() {
        window.location.href = "english/profile.php?id=" + idPerfil;
    }
  </script>
</body>
</html>
